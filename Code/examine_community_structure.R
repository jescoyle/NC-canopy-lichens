...
source('./Analysis/GitHub/NC-canopy-lichens/Code/CFT_functions.R')
...
# Load sample mean traits generated by model_func_traits.R
...
write.csv(sampXsp_macro, './Data/Derived Tables/comm_macro.csv', row.names=T)
...
write.csv(sampXsp_thalli, './Data/Derived Tables/comm_thalli.csv', row.names=T)
...
# Calculate distance matrix between samples using modified Hellinger distance (defined in CFT_functions.R)
...
## Constrain by each environmental variable separately
...
# Partial db-RDA: uses Y.p and conditions on site (year)
ord2_byvar = lapply(env_vars, function(x) rda(Y.p~X[,x]+Condition(W)))
names(ord2_byvar) = env_vars

# Partial CCA: uses Y and conditions on site (year)
